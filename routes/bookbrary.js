...
app.post('/search', (req, res) => {
    ...
    if (req.body.busqueda.trim().length > 0) {
        ...
        libro.find({}, { codigo: 1, descripcion: 1, titulo: 1, autor: 1, categoria: 1, genero: 1, formato: 1, precio: 1, stock: 1, img: 1, editorial: 1 }, (err, books) => {
            ...
            let token_consulta = tokenizer.tokenize(consulta)
            token_consulta = sw.removeStopwords(token_consulta, sw.es)
            token_consulta.forEach(function(value, i, vector) {
            ...
            books.forEach((element, i, myarray) => {
                    ...
                    let desc = coseno(token_consulta.join(' '), element.descripcion)
                   /* if (desc > 0.05) {
                        desc = 0.11
                    }*/
                    //
                    ...
                    let tittle = natural.DiceCoefficient(token_consulta.join(' '), token_title.join(' '))
                    if (tittle < 0.25) {
                       tittle = 0
                    } else {
                        tittle = tittle
                    }
                    let autor = natural.DiceCoefficient(token_consulta.join(' '), element.autor)
                    if (autor < 0.3) {
                       autor = 0
                    } else {
                        autor = autor
                    ...
                    let categoria = jaccard(token_consulta.join(' '), element.categoria.join(' '))
                    let generos = jaccard(token_consulta.join(' '), element.genero.join(' '))
                    /*if(generos>0.1){
                        console.log(element.genero.join(' '));
                    }*/
                        //let generos = natural.DiceCoefficient(consulta, element.genero.join(' ')) * 0.2
                    let editorial = natural.DiceCoefficient(token_consulta.join(' '), element.editorial)
                    if (editorial < 0.6) {
                        editorial = 0
                    } else {
                        editorial = editorial
                    }
                    var scoretemp = tittle + autor + categoria + desc + generos + editorial
                    myarray[i].score = scoretemp
            ...
            books.forEach((element, i, myarray) => {
                ...
                //console.log(element.titulo , element.score);
...
app.get('/recommended', async(req, res) => {
    ...
    let libros = await libro.find({
            ...
            let percent_autor = 0.3
            let percent_genero = 0.5
            let percent_categoria = 0.2
            let scores = []
